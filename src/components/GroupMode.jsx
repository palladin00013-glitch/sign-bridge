...
export default function GroupMode() {
  ...
  return <div className="flex flex-col gap-space-md px-margin pb-space-lg pt-space-sm"><div className="flex items-center gap-space-xs"><span className="h-2.5 w-2.5 animate-ping rounded-full bg-secondary" /><span className="text-sm font-semibold text-secondary">Multi-Vision Active</span></div><div className="relative min-h-[290px] w-full overflow-hidden rounded-2xl bg-surface-container-lowest" style={{ aspectRatio: '4/3' }}><video ref={videoRef} autoPlay muted playsInline className="absolute inset-0 h-full w-full object-cover opacity-85" /><div className="relative z-10 grid h-full grid-cols-2 gap-space-sm p-space-xs"><PersonPanel label="P1 · Left" colorClass="text-primary" sign={left?.label || left} /><PersonPanel label="P2 · Right" colorClass="text-secondary" sign={right?.label || right} /></div></div><div className="flex items-start gap-3 rounded-xl bg-surface-container-low p-space-md"><Icon name="info" className="text-primary" size={20} /><p className="text-xs text-on-surface-variant">Constrained demo: left/right frame split, not real person tracking. Works best with two people on opposite sides of frame with minimal overlap.</p></div></div>
}
